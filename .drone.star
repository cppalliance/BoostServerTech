...
def main(ctx):
    return generate(
        ...
        ['gcc >=4.8',
         ...
         # 'freebsd-gcc latest',
         'apple-clang *',
         # 'arm64-clang latest', # vcpkg: /bin/sh: 1: /usr/local/clang+llvm-11.0.0/bin/llvm-ar: Exec format error
         's390x-clang latest',
         # 'freebsd-clang latest', # pkg: Insufficient privileges to install packages
         # 'x86-msvc latest'
         ],
        # Standards
        '>=14',
